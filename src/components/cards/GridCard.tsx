...
export const GridCard = ({ children, className, href, isButton = false }: BentoCardProps) => {
  return (
    <Link
      href={href ?? ""}
      className={`group relative flex overflow-hidden rounded-xl border bg-zinc-50/50 transition-all duration-300 hover:bg-white/80 dark:bg-bgDarkSecondary ${className}`}
    >
