import { GridCard } from "@/components/cards/GridCard"
import Image from "next/image"
import { Suspense } from "react"
import { PiClock, PiClockAfternoon, PiCode, PiSunBold } from "react-icons/pi"
import { useTranslations } from "next-globe-gen"
import { ReactNode } from "react"

interface FavoriteSongContent {
  title: string
  descriptionBefore: string
  highlight: string
  descriptionAfter: string
  imageAlt: string
}

const FavoriteSongItem = ({ content }: { content: FavoriteSongContent }) => (
  <GridCard className="flex min-h-[200px] flex-col p-4 md:min-h-[250px] lg:row-span-2 lg:min-h-[300px]">
    <div className="sizeSubtitle flex flex-col gap-2">
      <h1 className="font-regular text-lg dark:text-textDarkPrimary">{content.title}</h1>
      <p className="text-base leading-relaxed text-textSecondary dark:text-textDarkSecondary">
        {content.descriptionBefore} <span className="font-semibold">{content.highlight}</span> {content.descriptionAfter}
      </p>
    </div>
    <div className="absolute -bottom-10 right-0 flex w-full items-center justify-center transition-all delay-100 duration-500 group-hover:-translate-y-5">
      <Image
        src="/assets/elements/element-novo-amor.png"
        alt={content.imageAlt}
        width={300}
        height={300}
        className="w-[180px] object-cover lg:w-full"
        priority
        sizes="(max-width: 768px) 180px, 300px"
      />
    </div>
  </GridCard>
)

interface PhotographyContent {
  title: string
  description: string
  images: { src: string; alt: string }[]
}

const PhotographyItem = ({ content }: { content: PhotographyContent }) => (
  <GridCard className="flex min-h-[150px] flex-col md:min-h-[180px] lg:col-span-3 lg:min-h-[150px]">
    <div className="sizeSubtitle flex flex-col gap-2 p-4">
      <h1 className="font-regular text-lg dark:text-textDarkPrimary">{content.title}</h1>
      <p className="text-base leading-relaxed text-textSecondary dark:text-textDarkSecondary">{content.description}</p>
      <div className="mt-2 flex gap-2">
        {content.images.map((img, index) => (
          <Image
            key={index}
            src={img.src}
            alt={img.alt}
            width={64}
            height={64}
            className="h-16 w-16 rounded-lg object-cover opacity-90"
            sizes="64px"
          />
        ))}
      </div>
    </div>
  </GridCard>
)

interface BookContent {
  title: string
  countLabel: string
  loadingLabel: string
  books: {
    src: string
    alt: string
    position: { left: string; top: string; rotate: string }
  }[]
}

const BooksItem = ({ content }: { content: BookContent }) => (
  <GridCard className="flex min-h-[200px] flex-col bg-[url('/assets/elements/element-dots.png')] bg-cover md:min-h-[250px] lg:col-span-3 lg:col-start-2 lg:row-span-2 lg:row-start-2 lg:min-h-[300px]">
    <div className="relative flex w-full flex-col p-6">
      <div className="sizeSubtitle mb-4 flex items-center justify-between">
        <h1 className="font-regular text-lg dark:text-textPrimary ">{content.title}</h1>
        <span className="text-textSecondary">{content.countLabel}</span>
      </div>

      <div className="relative h-[200px] md:h-[300px]">
        <Suspense fallback={<div>{content.loadingLabel}</div>}>
          {content.books.map((book, index) => (
            <div
              key={index}
              className={`absolute ${book.position.left} ${book.position.top} ${book.position.rotate} transition-transform duration-300 hover:scale-110`}
            >
              <div className="h-32 w-24 overflow-hidden rounded-lg shadow-lg md:h-40 md:w-32">
                <Image
                  src={book.src}
                  alt={book.alt}
                  width={128}
                  height={160}
                  className="h-full w-full object-cover"
                  sizes="(max-width: 768px) 96px, 128px"
                />
              </div>
            </div>
          ))}
        </Suspense>
      </div>
    </div>
  </GridCard>
)

interface ReadingItem {
  number: string
  title: string
  status: string
}

interface ReadingContent {
  title: string
  subtitle: string
  items: ReadingItem[]
  progressLabel: string
  progressValue: string
}

const ReadingProgressItem = ({ content }: { content: ReadingContent }) => (
  <GridCard className="flex min-h-[150px] flex-col p-6 md:min-h-[180px] lg:col-start-5 lg:row-span-3 lg:row-start-1 lg:min-h-[450px]">
    <div className="relative flex h-full flex-col">
      <div className="sizeSubtitle mb-8">
        <h3 className="font-regular text-lg tracking-tight text-textPrimary dark:text-textDarkPrimary">{content.title}</h3>
        <p className="mt-2 text-base leading-relaxed text-textSecondary dark:text-textDarkSecondary">{content.subtitle}</p>
      </div>

      <div className="mt-6 flex flex-col gap-5">
        {content.items.map((item, index) => (
          <div key={index} className="group relative flex items-center gap-4 rounded-xl bg-gray-50/50 p-4 transition-all hover:bg-gray-50">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-100 text-gray-700 transition-transform group-hover:scale-105">
              <span className="text-sm font-medium">{item.number}</span>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-700 dark:text-textDarkPrimary">{item.title}</p>
              <p className="mt-1 text-xs text-gray-400 dark:text-textDarkSecondary">{item.status}</p>
            </div>
            <div className="absolute right-4 text-gray-300 transition-colors group-hover:text-gray-400">
              <span className="text-xs">→</span>
            </div>
          </div>
        ))}
      </div>

      <div className="mt-auto">
        <div className="mt-8 space-y-3">
          <div className="flex items-center justify-between text-xs text-gray-600 dark:text-textDarkSecondary">
            <span>{content.progressLabel}</span>
            <span>{content.progressValue}</span>
          </div>
          <div className="h-1 w-full overflow-hidden rounded-full bg-gray-100">
            <div className="h-full w-3/5 rounded-full bg-gray-300 transition-all"></div>
          </div>
        </div>
      </div>

      <div className="absolute bottom-0 right-0 h-40 w-40 rounded-full bg-gradient-to-br from-gray-50 to-gray-100/50 blur-3xl" />
    </div>
  </GridCard>
)

interface DailyRoutineItem {
  icon: ReactNode
  text: string
}

interface DailyRoutineContent {
  title: string
  items: DailyRoutineItem[]
}

const DailyRoutineCard = ({ content }: { content: DailyRoutineContent }) => (
  <GridCard className="flex min-h-[150px] flex-col gap-4 p-6 md:min-h-[180px] lg:row-start-3 lg:min-h-[150px]">
    <div className="flex flex-col gap-4">
      <h3 className="text-lg font-medium text-gray-700 dark:text-textDarkPrimary">{content.title}</h3>
      <div>
        {content.items.map((routine, index) => (
          <div key={index} className="group flex items-center gap-4 rounded-lg p-2 transition-all hover:bg-gray-50">
            <span className="flex items-center justify-center rounded-lg bg-[#3C3C3F] p-2 text-textDarkPrimary dark:bg-backgroundPrimary dark:text-textPrimary">
              {routine.icon}
            </span>
            <p className="text-sm text-gray-600 dark:text-textDarkSecondary">{routine.text}</p>
          </div>
        ))}
      </div>
    </div>
  </GridCard>
)

// Main component
export function MoreSection() {
  const t = useTranslations("about")

  const favoriteSongContent: FavoriteSongContent = {
    title: t("more.favoriteSong.title"),
    descriptionBefore: t("more.favoriteSong.descriptionBefore"),
    highlight: t("more.favoriteSong.highlight"),
    descriptionAfter: t("more.favoriteSong.descriptionAfter"),
    imageAlt: t("more.favoriteSong.imageAlt"),
  }

  const photographyContent: PhotographyContent = {
    title: t("more.photography.title"),
    description: t("more.photography.description"),
    images: [
      { src: "/assets/images/home-1.jpg", alt: t("more.photography.altMorning") },
      { src: "/assets/images/home-2.jpg", alt: t("more.photography.altRainy") },
      { src: "/assets/images/home-3.jpg", alt: t("more.photography.altQuiet") },
    ],
  }

  const booksContent: BookContent = {
    title: t("more.books.title"),
    countLabel: t("more.books.countLabel"),
    loadingLabel: t("more.books.loading"),
    books: [
      {
        src: "/assets/images/cb-SangAlkemis.webp",
        alt: t("more.books.altSangAlkemis"),
        position: { left: "left-[10%]", top: "top-[10%]", rotate: "-rotate-12" },
      },
      {
        src: "/assets/images/cb-Financial.webp",
        alt: t("more.books.altFinancial"),
        position: { left: "left-[35%]", top: "top-[20%]", rotate: "rotate-6" },
      },
      {
        src: "/assets/images/cb-Silent.webp",
        alt: t("more.books.altSilent"),
        position: { left: "right-[20%]", top: "top-[5%]", rotate: "-rotate-3" },
      },
      {
        src: "/assets/images/cb-ThePsychologyOfMoney.webp",
        alt: t("more.books.altPsychology"),
        position: { left: "left-[25%]", top: "bottom-[10%]", rotate: "rotate-[-8deg]" },
      },
      {
        src: "/assets/images/cb-FilosofiTeras.webp",
        alt: t("more.books.altFilosofi"),
        position: { left: "right-[15%]", top: "bottom-[20%]", rotate: "rotate-12" },
      },
    ],
  }

  const readingContent: ReadingContent = {
    title: t("more.reading.title"),
    subtitle: t("more.reading.subtitle"),
    items: [
      { number: "01", title: t("more.reading.items.designSystems.title"), status: t("more.reading.items.designSystems.status") },
      { number: "02", title: t("more.reading.items.webArchitecture.title"), status: t("more.reading.items.webArchitecture.status") },
      { number: "03", title: t("more.reading.items.apiDesign.title"), status: t("more.reading.items.apiDesign.status") },
    ],
    progressLabel: t("more.reading.progress.label"),
    progressValue: t("more.reading.progress.value"),
  }

  const dailyRoutineContent: DailyRoutineContent = {
    title: t("more.dailyRoutine.title"),
    items: [
      { icon: <PiSunBold className="size-6" />, text: t("more.dailyRoutine.items.wake") },
      { icon: <PiCode className="size-6" />, text: t("more.dailyRoutine.items.code") },
      { icon: <PiClockAfternoon className="size-6" />, text: t("more.dailyRoutine.items.exercise") },
    ],
  }

  return (
    <section className="relative w-full overflow-hidden">
      <div className="my-8 text-center md:my-12 lg:my-16">
        <h2 className="border-y py-2 text-sm font-medium uppercase tracking-wider text-accentColor">{t("more.label")}</h2>
        <div className="sizeTitle mx-auto flex w-full items-center justify-center border-b py-4 text-textPrimary md:py-5 lg:text-[40px]">
          <h1 className="max-w-sm text-xl dark:text-textDarkPrimary md:text-2xl lg:text-3xl">{t("more.title")}</h1>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4 px-4 md:gap-6 md:px-6 lg:grid-cols-5 lg:grid-rows-2 lg:gap-2 lg:px-5">
        <FavoriteSongItem content={favoriteSongContent} />
        <PhotographyItem content={photographyContent} />
        <BooksItem content={booksContent} />
        <ReadingProgressItem content={readingContent} />
        <DailyRoutineCard content={dailyRoutineContent} />
      </div>
    </section>
  )
}
