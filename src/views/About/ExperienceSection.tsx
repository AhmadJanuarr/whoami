...
const ExperienceCard = ({ company, period, role, description }: ExperienceCardProps) => (
  ...
  return (
    <section className="relative w-full overflow-hidden">
      <div className="mb-12 text-center md:mb-16">
        <h2 className="border-y py-2 text-sm font-medium uppercase tracking-wider text-accentColor">{t("experience.label")}</h2>
        <div className="sizeTitle mx-auto flex w-full items-center justify-center border-b py-4 text-textPrimary dark:text-textDarkPrimary md:py-5 lg:text-[40px]">
          <h1 className="max-w-sm text-2xl md:text-3xl lg:text-[40px]">{t("experience.title")}</h1>
        </div>
      </div>
        ...
        <div className="space-y-8 md:space-y-0">
          {EXPERIENCE_LIST.map((experience, index) => (
            <div key={index} className={index < EXPERIENCE_LIST.length - 1 ? "pb-12 md:pb-20" : ""}>
              <ExperienceCard
                company={t(experience.companyKey as Parameters<typeof t>[0])}
                period={t(experience.periodKey as Parameters<typeof t>[0])}
                role={t(experience.roleKey as Parameters<typeof t>[0])}
                description={experience.descriptionKeys.map((key) => t(key as Parameters<typeof t>[0]))}
              />
            </div>
          ))}
        </div>
